...
The live editorial database contains 530 records: a 30-record founding pilot and 500 exact-text draft candidates extracted from 50 public-domain books. The pilot follows a strict editorial rule: money, payment, wages, debt, value, or currency must be necessary to the passage rather than incidental. Twenty-eight pilot records are source-linked and rights-cleared for the queue. Two Federal Writers’ Project leads and all 500 mechanically selected candidates remain drafts and cannot publish automatically without an editor changing their workflow status.
...
The public design uses the Tidal Ledger palette: aqua, deep green, violet, and vermilion. The quotation sits on an uninterrupted reading field while a separate edge channel moves on five slow, asynchronous 8-13 second tides. On mobile, the channel becomes a quieter three-bar composition contained beside the upper quotation. Quotation words resolve gradually in three-word groups on first load. Soon after the last words settle, groups recede in a newly shuffled order on every cycle. Each phrase fades for 1.8 seconds, remains recessed for 3.2 seconds, returns over 1.8 seconds, and rests before another phrase moves. This creates an erasure-poem rhythm without changing the source text. The Share action generates a square PNG broadside locally from the exact quotation and attribution, with controls to share, copy, or download the image. Readers who prefer reduced motion receive the complete, static text immediately. IBM Plex Serif and IBM Plex Mono are self-hosted under their OFL licenses in `site/assets/fonts/`.
