...
function revealQuote(value) {
  ...
  return tokens.map((token) => {
    ...
    words += 1;
    const revealDelay = 120 + groupIndex * 520;
...
function startQuoteErasure() {
  ...
  const recede = () => {
    ...
    erasureRestoreTimer = setTimeout(() => phrase.forEach((word) => word.classList.remove('is-receding')), 5000);
    previousPhrase = phrase;
    phraseIndex = (phraseIndex + 1) % phrases.length;
    if (phraseIndex === 0) order = shuffle(phrases, previousPhrase);
    erasureTimer = setTimeout(recede, phraseIndex === 0 ? 12000 : 8000);
  };

  erasureTimer = setTimeout(recede, Math.max(5200, phrases.length * 520 + 2800));
}
